...
mod tests {
    ...
    #[test]
    fn single_char() {
        if "b".to_owned().match_from_str("abc").is_some() {
            panic!("only match from start");
        }
    ...
    #[test]
    fn digit_string() {
        if DecimalDigitString.match_from_str("a1").is_some() {
            panic!()
        };
